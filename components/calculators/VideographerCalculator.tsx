...
interface VideographerService {
  eventType: 'wedding' | 'corporate' | 'documentary' | 'event' | 'custom';
  duration: number;
...
export function VideographerCalculator() {
  const [eventType, setEventType] = useState<VideographerService['eventType']>('wedding');
  const [duration, setDuration] = useState<number>(4);
  const [packageLevel, setPackageLevel] = useState<VideographerService['packageLevel']>('standard');
  const [baseRate, setBaseRate] = useState<number>(0);
  const [serviceQuality, setServiceQuality] = useState<VideographerService['serviceQuality']>('good');
  const [result, setResult] = useState<VideographerService | null>(null);
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <Select
            value={eventType}
            onValueChange={(value: VideographerService['eventType']) => setEventType(value)}
          >
            <SelectTrigger>
        ...
        <div className="space-y-2">
          ...
          <Select
            value={packageLevel}
            onValueChange={(value: VideographerService['packageLevel']) => setPackageLevel(value)}
          >
            <SelectTrigger>
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            onValueChange={(value: VideographerService['serviceQuality']) => setServiceQuality(value)}
          >
            <SelectTrigger>
      ...
      {result && (
        <Card>
          <CardHeader>
            <CardTitle>Videography Service Breakdown</CardTitle>
          </CardHeader>
